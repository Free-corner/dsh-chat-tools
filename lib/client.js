...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CSS = [
			...
			".dshct-uiSlider{flex:1 1 auto;min-width:0;height:18px;accent-color:var(--dsw-alias-accent,#4f7cff);cursor:pointer}",
			...
			".dshct-tick{display:block;height:5px;border-radius:2.5px;background:var(--dsw-alias-label-tertiary,rgba(120,120,130,.65));opacity:.55;flex:none}",
			".dshct-rail:hover .dshct-tick{opacity:.85}",
			".dshct-tickActive{background:var(--dsw-alias-accent,#4f7cff);opacity:1}",
			...
			".dshct-railR .dshct-tick{background:var(--dsw-alias-label-tertiary,rgba(120,120,130,.65));opacity:.55}",
			".dshct-railR:hover .dshct-tick{opacity:.85}",
			".dshct-railR .dshct-tickActive{background:var(--dsw-alias-accent,#4f7cff);opacity:1}",
			/* Reduced motion */
				...
				uiMenu = React.createElement("div", {
					...
					React.createElement("div", { className: "dshct-uiRow" },
						...
						React.createElement("input", {
							...
							onChange: function (e) { setWidthPct(Number(e.target.value)); },
						}),
			...
			if (fileOutline && fileRailRect && fileOutline.headings.length > 0) {
				...
				if (filePopOpen) {
					fpop = React.createElement("div", {
						className: "dshct-railPop",
						style: { right: (window.innerWidth - fileRailRect.left + 6) + "px", top: fileRailRect.top + "px" },
						onMouseEnter: function () { setFilePopOpen(true); },
						onMouseLeave: function () { setFilePopOpen(false); },
					},
				...
				railR = React.createElement("div", {
					...
					onMouseLeave: function () { setFileHover(-1); setFilePopOpen(false); },
				},
