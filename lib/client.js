	...
	factory: (require) => {
		...
		var CSS = [
			...
			/* Dropdown popover: anchored under the header bar, never fixed to the
			   ...
			   covering sidebar/overlay plugins. */
			".dshct-menu{position:absolute;top:calc(100% + 6px);right:0;z-index:120;display:flex;flex-direction:column;box-sizing:border-box;min-width:220px;max-width:min(620px,calc(100vw - 32px));max-height:min(440px,calc(100vh - 120px));background:var(--dsw-alias-bg-layer-1);border:1px solid var(--dsw-alias-border-l2);border-radius:12px;box-shadow:var(--dsw-shadow-lv3,0 8px 30px rgba(0,0,0,.18));overflow:hidden}",
			...
			/* Outline ticks: pill-shaped gradient bars — soft gray by default,
			   accent gradient + glow for the active heading. All ticks share
			   the same thickness; the active one is distinguished by color and
			   glow only (never scaled, so the bar heights stay uniform). */
			".dshct-railTicks{display:flex;flex-direction:column;align-items:flex-start;gap:9px;pointer-events:none}",
			".dshct-tick{display:block;height:4px;border-radius:999px;flex:none;opacity:.72;background:var(--dsw-alias-label-tertiary,rgba(120,120,130,.7));background:linear-gradient(90deg,var(--dsw-alias-label-tertiary,rgba(120,120,130,.8)) 0%,var(--dsw-alias-label-tertiary,rgba(120,120,130,.3)) 100%);transition:opacity .15s,box-shadow .15s,background .15s}",
			".dshct-rail:hover .dshct-tick{opacity:1}",
			".dshct-tickActive{opacity:1;background:var(--dsw-alias-accent,#4f7cff);background:linear-gradient(90deg,var(--dsw-alias-accent,#4f7cff) 0%,color-mix(in srgb,var(--dsw-alias-accent,#4f7cff) 35%,transparent) 100%);box-shadow:0 0 8px color-mix(in srgb,var(--dsw-alias-accent,#4f7cff) 55%,transparent)}",
			".dshct-railPop{position:fixed;z-index:130;display:flex;flex-direction:column;min-width:200px;max-width:340px;max-height:60vh;overflow-y:auto;background:var(--dsw-alias-bg-layer-1);border:1px solid var(--dsw-alias-border-l2);border-radius:10px;box-shadow:var(--dsw-shadow-lv3,0 8px 30px rgba(0,0,0,.18));padding:6px;pointer-events:auto}",
			...
			".dshct-railR .dshct-tick{background:var(--dsw-alias-label-tertiary,rgba(120,120,130,.7));background:linear-gradient(270deg,var(--dsw-alias-label-tertiary,rgba(120,120,130,.8)) 0%,var(--dsw-alias-label-tertiary,rgba(120,120,130,.3)) 100%);opacity:.72}",
			".dshct-railR:hover .dshct-tick{opacity:1}",
			".dshct-railR .dshct-tickActive{opacity:1;background:var(--dsw-alias-accent,#4f7cff);background:linear-gradient(270deg,var(--dsw-alias-accent,#4f7cff) 0%,color-mix(in srgb,var(--dsw-alias-accent,#4f7cff) 35%,transparent) 100%);box-shadow:0 0 8px color-mix(in srgb,var(--dsw-alias-accent,#4f7cff) 55%,transparent)}",
			/* Reduced motion */
		 ...
		 * @param instant - true for keyboard/nav-button jumps: set the scroll
		 *   position directly with no animation. Smooth scrolling over a long
		 *   session animates for a long time and queued jumps feel stuck, so
		 *   quick navigation must skip it. Defaults to false (smooth).
		 */
		function jumpTo(key, instant) {
			if (typeof document === "undefined") return;
			...
			// Instant: find the conversation scrollport and set scrollTop
			// directly. scrollIntoView({behavior:'instant'}) is unreliable
			// across browsers for deeply nested scroll containers, while a
			// manual offset is synchronous and exact.
		...
		function ChatTools(props) {
			...
			if (outline && railRect && outline.headings.length > 0) {
				...
				var TICK_PITCH = 14; // tick height 5px + gap 9px
				var ticks = [];
				var popItems = [];
				...
				function hitIndexFromEvent(e) {
					// rail has padding-top 6px; each tick occupies 5px + 9px gap.
