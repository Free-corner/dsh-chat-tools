	...
	factory: (require) => {
		...
		var CSS = [
			...
			/* Outline ticks: pill-shaped gradient bars — soft gray by default,
			   accent gradient + glow for the active heading, subtle scale-up on
			   hover. Gradient direction mirrors each side's alignment. */
			".dshct-railTicks{display:flex;flex-direction:column;align-items:flex-start;gap:9px;pointer-events:none}",
			".dshct-tick{display:block;height:4px;border-radius:999px;flex:none;opacity:.72;background:var(--dsw-alias-label-tertiary,rgba(120,120,130,.7));background:linear-gradient(90deg,var(--dsw-alias-label-tertiary,rgba(120,120,130,.8)) 0%,var(--dsw-alias-label-tertiary,rgba(120,120,130,.3)) 100%);transition:opacity .15s,transform .15s,box-shadow .15s,background .15s}",
			".dshct-rail:hover .dshct-tick{opacity:1}",
			".dshct-tickActive{opacity:1;background:var(--dsw-alias-accent,#4f7cff);background:linear-gradient(90deg,var(--dsw-alias-accent,#4f7cff) 0%,color-mix(in srgb,var(--dsw-alias-accent,#4f7cff) 35%,transparent) 100%);box-shadow:0 0 8px color-mix(in srgb,var(--dsw-alias-accent,#4f7cff) 55%,transparent);transform:scaleY(1.35)}",
			...
			".dshct-railR .dshct-tickActive{opacity:1;background:var(--dsw-alias-accent,#4f7cff);background:linear-gradient(270deg,var(--dsw-alias-accent,#4f7cff) 0%,color-mix(in srgb,var(--dsw-alias-accent,#4f7cff) 35%,transparent) 100%);box-shadow:0 0 8px color-mix(in srgb,var(--dsw-alias-accent,#4f7cff) 55%,transparent);transform:scaleY(1.35)}",
			/* Reduced motion */
