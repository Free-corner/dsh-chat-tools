...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		async function loadOlderAndJump(session, key, onProgress) {
			...
			// Already in the window? Jump straight away.
			var el = document.querySelector("[data-chat-anchor-key=" + quoteAttr(key) + "]");
			if (el) {
		...
		function ChatTools(props) {
			...
			React.useEffect(function () {
				function onKey(e) {
					...
					if (e.key === "ArrowUp") {
						if (q > 0 && list[q - 1]) onQClickRef.current(list[q - 1], q - 1);
					} else {
						if (q < list.length - 1 && list[q + 1]) onQClickRef.current(list[q + 1], q + 1);
					}
				}
			...
			// outline update below).
			React.useEffect(function () {
				var ticking = false;
				function update() {
					var scrollBody = document.querySelector("[data-conversation-scroll]");
					if (!scrollBody || questions.length === 0) return;
					...
					for (var j = 0; j < questions.length; j++) {
						if (questions[j].key === lastUserKey) {
							setCurrentQ(j);
							break;
						}
					}
					...
					React.createElement("div", { className: "dshct-navRow" },
						React.createElement("button", {
							...
							onClick: function () {
								...
								if (allQuestions[q]) onQuestionClick(allQuestions[q], q);
							},
						}, "\u2191 上一个"),
						React.createElement("button", {
							type: "button",
