...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CSS = [
			...
			/* Outline ticks: pill-shaped gradient bars, ALL with identical
			   size and opacity. The active heading is distinguished by color
			   only (gray → accent) — no glow, no scale, no opacity change, so
			   every bar reads the same thickness. */
			".dshct-railTicks{display:flex;flex-direction:column;align-items:flex-start;gap:9px;pointer-events:none}",
			".dshct-tick{display:block;height:4px;border-radius:999px;flex:none;opacity:1;background:linear-gradient(90deg,var(--dsw-alias-label-tertiary,rgba(120,120,130,.55)) 0%,var(--dsw-alias-label-tertiary,rgba(120,120,130,.25)) 100%);transition:background .15s}",
